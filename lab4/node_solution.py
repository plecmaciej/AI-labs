...
class Node:
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for split_idx in possible_splits:
            left_labels = y[:split_idx + 1]
            right_labels = y[split_idx + 1:]

            left_positives = np.sum(left_labels)
            left_negatives = len(left_labels) - left_positives
            gini_left = 1 - (left_positives / len(left_labels)) ** 2 - (left_negatives / len(left_labels)) ** 2

            right_positives = np.sum(right_labels)
            right_negatives = len(right_labels) - right_positives
            gini_right = 1 - (right_positives / len(right_labels)) ** 2 - (right_negatives / len(right_labels)) ** 2

            weighted_gini = 1 - len(left_labels) / len(y) * gini_left - len(right_labels) / len(y) * gini_right
            # Update best gain if this split is better
    ...
    def find_best_split(self, X, y, feature_subset):
        ...
        if feature_subset is not None:
            selected_features = np.random.choice(X.shape[1], feature_subset)
        else:
            default_subset_size = int(np.sqrt(X.shape[1])) + 1
