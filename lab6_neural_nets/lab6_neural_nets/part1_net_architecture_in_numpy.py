...
def zad1_single_neuron(student_id):
    ...
    #model.W[:, 0] =  np.random.randn(n_features)  # Inicjalizacja losowych wag
...
def zad2_two_layer_net(student_id):
    ...
    class SimpleTwoLayerNetwork:
        def __init__(self, n_in, n_hidden, n_out):
            ...
            self.hidden_layer = DenseLayer(n_in=n_hidden, n_out=n_hidden, f_act=hardlim)
            # Inicjalizacja warstwy wyjściowej
            self.output_layer = DenseLayer(n_in=n_in, n_out=n_out, f_act=hardlim)
    ...
    w1 = -2
    w2 = -1
    model.hidden_layer.W[:, 0] = [w1, w2]      # wagi neuronu h1
    model.hidden_layer.W[:, 1] = [-1.6,-0.666]     # wagi neuronu h2
    model.hidden_layer.b[:] = 0.5         # biasy neuronów h1 i h2
    model.output_layer.W[:, 0] = [2,4]     # wagi neuronu wyjściowego
...
if __name__ == '__main__':
    ...
    student_id = 193755         # Twój numer indeksu, np. 102247
